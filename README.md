...
普通总结器更像压缩原文，`大学老师.skill` 更强调把课程材料整理成 `老师会怎么讲 / 这门课会怎么考 / 哪里容易扣分` 这三类线索。

### 没有往年题还能不能用？

可以用，但会更保守。没有往年题时，结果会更偏课程结构总结和高频提醒，而不是强行给出像押题一样的判断。

### 只有 PPT 值不值得导入？

可以导入，但效果通常不如 `PPT + 作业 + 转写 + 往年题` 的组合。只有 PPT 时，项目更适合帮你压缩重点，不适合过度期待命题偏好分析。

### 它会不会一本正经胡说八道？

会尽量避免。当前设计就是材料不足时主动降级，不把低证据内容包装成高置信度结论。
